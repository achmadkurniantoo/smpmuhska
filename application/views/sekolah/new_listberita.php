    ...
    <?php foreach ($results as $key) { ?>
        <div class="row">
            <div class="col-md-7">
                    ...
                    ?>
                <a href="<?php echo base_url().'sekolah/baca_akademik/'.$key->kategori. "/".$key->id_akademik ?>">
                    <?php if ($matches != NULL) { ?>
            ...
            <div class="col-md-5">
                <h3><?=$key->judul?></h3>
                <h4>POSTED ON <?=$key->tgl_posting?>  / <a href="<?php echo base_url().'sekolah/akademik/'. $key->kategori; ?>" rel="category tag"> <?=$key->kategori?></a></h4>
                <?php $aa = strip_tags(html_entity_decode($key->isi));?>
                <div class="exp"><?=substr($aa, 0,400)?>...</div>
                 <div align="right"><?=anchor('sekolah/baca_akademik/'.$key->kategori. "/".$key->id_akademik ,'Baca Selengkapnya..',['class'=>'btn btn-success'])?></div>
            </div>
        </div>
        <hr>
    <?php  }?>
